...
public class PianoView extends View{
    public static final int NB = 14; // 건반 개수
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
       ...
       // int count = 15;
        ...
        for (int i = 0; i < NB; i++) {
            ...
            whites.add(new Key(sound, color_white ,rect));
            // 위에 코드 수정중 ...
            // octave 설정을 받아와서 넣으면 될 듯
            // sound 는 1부터 7 까지 이므로

            if (i != 0  &&   i != 3  &&  i != 7  &&  i != 10) {
                rect = new RectF((float) (i - 1) * keyWidth + 0.5f * keyWidth + 0.25f * keyWidth, 0,
                        (float) i * keyWidth + 0.25f * keyWidth, 0.67f * height);
                blacks.add(new Key(sound+15, color_black ,rect));
                sound++;
            }
        }
    }
